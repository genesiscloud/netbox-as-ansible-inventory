...
class NetboxAsInventory(object):
    ...
    def generate_inventory(self):
        ...
        inventory_dict = dict()
        netbox_hosts_list = self.get_hosts_list(self.api_url, self.api_token,
                                                self.host)
        if netbox_hosts_list:
            ...
            for current_host in netbox_hosts_list:
                ...
                if server_name:
                    ...
                    host_vars = self.get_host_vars(current_host,
                                                   self.host_vars)
                    inventory_dict = self.update_host_meta_vars(inventory_dict,
                                                                server_name,
                                                                host_vars)
        return inventory_dict
